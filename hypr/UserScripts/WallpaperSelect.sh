...
set_sddm_wallpaper() {
    sleep 1
    sddm_simple="/usr/share/sddm/themes/sugar-candy"

    if [ -d "$sddm_simple" ]; then
        # DEBUG: Check if we're getting here
        echo "DEBUG: SDDM directory exists, showing prompt" >> /tmp/wallpaper_debug.log
        date >> /tmp/wallpaper_debug.log
        ...
        if yad --info --text="Set current wallpaper as SDDM background?\n\nNOTE: This only applies to Sugar Candy Theme" \
          --text-align=left \
          ...
          --button="no:1"; then
            ...
            echo "DEBUG: User clicked YES, calling SDDM script" >> /tmp/wallpaper_debug.log
            ...
            echo "DEBUG: After exec call" >> /tmp/wallpaper_debug.log
    
        else
            echo "DEBUG: User clicked NO or timeout" >> /tmp/wallpaper_debug.log
        fi
    else
        echo "DEBUG: SDDM directory NOT found: $sddm_simple" >> /tmp/wallpaper_debug.log
    fi
}
...
apply_image_wallpaper() {
  ...
  swww img -o "$focused_monitor" "$image_path" $SWWW_PARAMS
   ...
   # UPDATE: Create symlink for rofi

    ln -sf "$image_path" ~/.config/rofi/.current_wallpaper
...
apply_video_wallpaper() {
 ...
 # UPDATE: Create symlink for rofi (use first frame or just path)
  ln -sf "$video_path" ~/.config/rofi/.current_wallpaper
